...
# Prompt user to enter book title. if book is not present.
# After user enters book title and not present in database. Prompt user to enter how many copies to add.
# Display message. x copy or copies have been added to inventory.
# if book is present. Display message. book already available. Return to sub menu.
...
while True:
    try:
        user_type = int(input("1. User\n2. Admin\nEnter choice: "))
        break
    except:
        print(f"\nError: invalid input - option is not available.\n")
        ...
        try:
            ...
            if admin_choice in [1,2,3,4]:
                if admin_choice == 1:
                    ...
                    if book_to_add not in book_list:
                        quantity_to_add = int(input(f"Enter how many copies of {book_to_add} to add: "))
                        if quantity_to_add == 1:
                            copy_message = 'copy'
                        elif quantity_to_add > 1:
                            copy_message = 'copies'
                        else:
                            print("Error")
                            continue
                        book_list[f"{book_to_add}"] = quantity_to_add
                        print(f"You have successfully added {quantity_to_add} {copy_message} of {book_to_add} to the database.")

                        # if quantity_to_add == 1:
                        #     book_list[book_to_add] = 1
                        #     print(f"You have successfully added {quantity_to_add} copy of {book_to_add} to the database.")
                        # elif quantity_to_add > 1:
                        #     book_list[f"{book_to_add}"] = quantity_to_add
                        #     print(f"You have successfully added {quantity_to_add} copies of {book_to_add} to the database.")
                        # else:
                        #     print("Error: ")
                    else:
                        print(f"{book_to_add} already in database. Update stock count under option 3. Update Inventory")
                        ...
                        if book_to_update_choice in [1,2,3]:
                            if book_to_update_choice == 1:
                                ...
                                if book_to_update_stock_count in book_list:
                                    modify_count_to = int(input("Enter how many copies to add: "))
                                    if modify_count_to > 0:
                                        book_list[book_to_update_stock_count] = modify_count_to
                                        print(f"Successfully added {modify_count_to} copies of'{book_to_update_stock_count}'.")
                                    else:
                                        print("Invalid entry: entry be a positive integer")
                                else:
                                    print(f"Sorry, {book_to_update_stock_count} is not available. Add to inventory")
